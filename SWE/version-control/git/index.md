...
- [Git worktrees for parallel AI agents](/SWE/version-control/git/git-worktrees-for-parallel-agents.md) — a linked working dir sharing one `.git` object store but with its own HEAD/index/tree; the primitive that isolates N parallel agents, pushing conflicts to visible merge time. `sb:8b9548` _(concept)_
