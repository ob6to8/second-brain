...
The move by which a glossary term that has outgrown its entry — accumulating
senses, claims, or citations that belong in the domain tree — relocates into
the taxonomy (e.g. `/knowledge/SWE/…`): the file moves, its stable id travels with it,
and a [pointer entry](/beliefs/glossary/pointer-entry.md) stub stays behind in
`/beliefs/glossary/` linking to the new home. The glossary is a staging layer, not a
terminal home. Defined by the
