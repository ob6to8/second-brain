...
In a **code knowledge graph**, functions become nodes and `CALLS`/`IMPORTS`/`EXTENDS`
become edges, letting a coding agent query architecture and impact ("what calls
this?") directly instead of reading files one by one. It is the supply-side structure
that tools like GitNexus and Codebase-Memory precompute (via
[Tree-sitter](/beliefs/glossary/tree-sitter.md)) and serve over
