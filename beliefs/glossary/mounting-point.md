...
The *role* that fills it — the [operating contract](/beliefs/glossary/operating-contract.md); a skill's procedure — is defined by the brain and would survive a harness change with only the output path moving. Separating role from mounting point is what lets `CLAUDE.md` be a [rendered aggregation](/beliefs/glossary/rendered-aggregation.md) compiled from `meta/policy/` while each `SKILL.md` stays a hand-authored source of truth.
