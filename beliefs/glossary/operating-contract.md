---
id: sb:1349cb
type: concept
title: operating contract
description: The root CLAUDE.md that every agent auto-loads at session start, codifying the brain's operative rules — a compiled artifact, not hand-edited.
provenance: "Agent-distilled glossary definition, pointer to the defining doc"
verified: false
tags: [glossary, governance]
sense: repo
timestamp: 2026-07-11
attribution:
  when: 2026-07-11T08:41:24+02:00
  channel: glossary
  agent: "Claude Code agent, /add-to-glossary"
  why: "term surfaced by the captured sources cited in Seen in (backfilled)"
---

# operating contract

The root `CLAUDE.md` that every agent auto-loads at session start, codifying the brain's operative rules. It is a [compiled contract](/beliefs/glossary/compiled-contract.md) — generated from [`meta/preamble.md`](/meta/preamble.md) and the policies under `meta/policy/` via `mix brain.contract`, never hand-edited. It binds *agents*, not the operator, who authors and ratifies its rules without being subject to them; and "contract" names this role, not the file — `CLAUDE.md` is only the harness [mounting point](/beliefs/glossary/mounting-point.md) where the role is delivered. See [`/CLAUDE.md`](/CLAUDE.md).

*Seen in:* [2026-07-05 OKF bootstrap thread](/meta/threads/2026-07-05-greenfield-okf-bootstrap-and-verification-layer.md), [2026-07-09 remove-email thread](/meta/threads/2026-07-09-remove-operator-email-from-contract.md), [2026-07-11 branch-deletion/contract thread](/meta/threads/2026-07-11-branch-deletion-policy-and-contract-as-abstraction.md)
