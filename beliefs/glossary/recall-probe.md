...
The metric it reports is typically [recall@k](/beliefs/glossary/recall-at-k.md). It scores a [gold set](/beliefs/glossary/gold-set.md) of query→target pairs against a search backend, catching retrieval that degrades as a corpus scales. In this brain it is realized as `mix brain.dedup_probe`: an offline, deterministic probe over [lexical search](/beliefs/glossary/lexical-search.md), whose plain-vs-[synonym-expanded](/beliefs/glossary/synonym-expansion.md) score gap quantifies the [deduplication](/beliefs/glossary/deduplication.md) recall that intake is missing.
