...
A session touches many matters, and each matter's thread of discussion — picked up, paused on an open question, resumed, resolved — is one strand. A strand can be routed in the [routing ledger](/beliefs/glossary/routing-ledger.md) yet still open, or closed and unrouted. A **dangling strand** is one whose state or unanswered question leaves work outstanding after the thread is frozen; the [session-init digest](/beliefs/glossary/session-init-digest.md) harvests these from all captured threads so they surface as open work in later sessions.
