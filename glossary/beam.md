...
The virtual machine that runs Erlang and Elixir. Its signature design is huge numbers of lightweight processes — each with its own isolated heap, preemptively scheduled, communicating only by message passing (the [actor model](/glossary/actor-model.md)) — which makes fault containment, per-process supervision, and hot code upgrades natural properties rather than add-ons. Those affordances pay off for long-running concurrent services; a one-shot batch program uses none of them, which is why this brain's [dependency-free](/glossary/dependency-free.md) tooling runs *on* the BEAM without being *a BEAM system*.
