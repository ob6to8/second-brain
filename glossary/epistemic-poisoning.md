...
The contamination of a shared knowledge base with false or attacker-shaped beliefs, typically via an agent that ingests untrusted external content and files it as fact. It is [prompt injection](/glossary/prompt-injection.md) escalated one level: instead of hijacking a single session, the attack corrupts the belief substrate every future agent reads, so one poisoned "fact" propagates to every decision that rests on it. Defenses are the knowledge base's immune system: immutable [provenance](/glossary/provenance.md), evidence-backed [verification grounding](/glossary/verification-grounding.md), quarantine states for unverified intake, and a single write choke point where checks cannot be bypassed.
