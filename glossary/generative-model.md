...
A model that captures how the data itself is distributed — the joint distribution over inputs (and, when present, labels) — so it can synthesize new samples that resemble the training data, and fill in or denoise partial inputs. This contrasts with a *discriminative* model, which learns only the conditional mapping from input to label. A [deep belief network](/glossary/deep-belief-network.md) is a generative model; so are VAEs, GANs, and diffusion and autoregressive models.
