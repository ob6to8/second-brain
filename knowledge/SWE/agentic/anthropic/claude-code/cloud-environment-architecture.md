...
tags: [claude-code, cloud, containers, orchestration, dev-environments, agentic]
...
in the captured `source` excerpts under [`sources/`](/knowledge/SWE/agentic/anthropic/claude-code/sources/index.md)
(see [Grounding](#grounding)).

Confidence per item: **[D]** documented in the official docs · **[E]** evidenced
by in-container forensics · **[I]** inference, unverified. A `→ em:xxxxxx` tag
points to the captured primary-source excerpt that grounds the item.

## The unit model

- **[D → em:863b32]** Users create an **environment** and it carries the
...
- **[D/E → em:eb418b]** An orchestrator maps session → environment → container,
...
- **[D/E → em:eb418b]** Container images are **pre-baked and cached across
...
- **[I]** Grounding **resolved** part of what was unverified: the snapshot is
  **per-environment** (confirmed → em:eb418b), and the **invalidation policy** is
  documented — the cache rebuilds when you change the setup script or allowed
  hosts, and on **~7-day expiry** (→ em:eb418b). Still unstated in the docs:
...
- **[D/E → em:eb418b]** Anything baked into the snapshot is **frozen at snapshot
  time**, not session time. Observed: the local `main` ref (created at image
  build, never advanced — [git fetch only moves remote-tracking refs](/knowledge/SWE/version-control/git/git-local-branches-dont-auto-advance-on-fetch.md)).
...
- **[D/E → em:3420c8]** The container is **ephemeral** — "Cloud sessions stop
...
- **[D/E → em:3f35e1]** Two settings/hook layers with different owners:
...
- **[D/E → em:564b8e]** Commit signing is effectively **deferred to push time**:
...
(→ em:863b32, em:3420c8) — reading as a *fresh clone per session*. Forensics found
...
(→ em:eb418b), so only the **first, cache-building** session gets a genuinely fresh
...
[`sources/`](/knowledge/SWE/agentic/anthropic/claude-code/sources/index.md):

- `em:863b32` — environments carry config; each session gets a fresh VM
- `em:eb418b` — environment caching (per-environment filesystem snapshots; ~7-day expiry)
- `em:3420c8` — environments reclaimed after inactivity
- `em:564b8e` — credentials and signing keys never inside the sandbox
- `em:3f35e1` — user-level settings don't carry to cloud; hooks come from repo and org
