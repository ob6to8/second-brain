defmodule ElixirMind.Attribution do
  @moduledoc """
  ...
  def list(root \\ File.cwd!(), opts \\ []) do
    {entries, _errors} = ElixirMind.Registry.scan(root)
    %{governance: governance} = governance_paths(root)
  ...
  defp frontmatter(root, rel_path) do
    case root |> Path.join(rel_path) |> File.read!() |> ElixirMind.Frontmatter.parse() do
