defmodule Mix.Tasks.Brain.SessionInit do
  @shortdoc "Print the session-init digest: open issues, active plans, dangling strands, top-3 priorities"
  ...
  Scans `meta/issues/` (status `open`), `meta/plans/` (status `proposed` /
  `accepted` / `in-progress`), and the `## Routing` ledgers under
  `meta/threads/` (rows in state `open`/`paused`, or with a dangling
  question), then prints a markdown digest ending in a heuristic top-3
