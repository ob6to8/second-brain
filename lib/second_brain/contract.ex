defmodule SecondBrain.Contract do
  @moduledoc """
  ...
  documents:
    ...
    * `meta/policy/*.md`  — the rules, as `type: policy` OKF docs (see `SecondBrain.Policy`)
  ...
  defp preamble_body(content) do
    case SecondBrain.Frontmatter.parse(content) do
      {:ok, %{body: body}} -> body
      {:error, _} -> content
    end
  end
