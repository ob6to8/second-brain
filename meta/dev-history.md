...
- file doctrine: capability-matched model selection
...
- Attribution plan accepted: ratify all five open questions
- Add the resource-attribution policy to the contract (build step 1)
- Verifier rule 8: attribution shape, from resolution, exemption placement (build step 2)
- Skill writers: every filing skill records the ingestion event (build step 3)
- Backfill attribution across the corpus; flip presence to mandatory (build step 4)
- Derive flow lineage from attribution.from + thread pr: anchors (build step 5)
- mix brain.attribution query surface; plan done (build step 6)
...
- Build mix brain.lineage; retrofit all flow docs with lineage frontmatter
...
- Regenerate registry after merge (canonical id→path view)
- news: auto-intake featured items into the bundle (§6) — complete the auto-intake plan
...
- capture: dedup recall probe + synonym intake thread; glossary +3 terms
...
- Tutorial: the session-init digest — sources, heuristic ranking, and the rank/judge split
- Flows: add the news and create-pull-request flow docs
- Docs-freshness warnings: link resolution + index coverage, warn-only in brain.verify
- Glossary: 7 new terms from the docs audit + field-comparison vocabulary; extend deduplication
...
- Add /elaborate skill: unpack a technical phrase in plain terms
- Persist elaborations: meta/elaborations/ genre, elaboration type, thread back-link
- Elaborate "two-directional materialize with unconditional orphan-block removal"
- Glossary: add materialize, excerpt log, orphan block (68 → 71)
- P1: materialize projects tags two-directionally, removing unfed sinks' logs
- P2: verifier gates `verified` to statement types (claim/note/concept)
- P3: hygiene batch — ledger scoping, terminator alignment, CRLF, warn-aware summary
- Flip code-review-toolchain-hardening to done
- Glossary: 8 new terms + 2 merges from the hardening session (93 → 101)
...
- Add render-contract flow doc and scenario test
- Re-audit orphaned-branches issue after merging main
- Record ratified branch cleanup: deletion blocked agent-side; file triage todo
- Preamble binding-party sentence; rendered-aggregations analysis; top-priority skills-compile plan
...
- Plan: dedup recall probe (gold set + mix brain.dedup_probe)
- Analysis: CB's epistemic overlay as a failure-chain stabilizer
- Epistemic overlay plan: add the execution-path addendum
...
- capture: 2026-07-10 add-to-glossary skill and PR wiring; seed the glossary
...
- Intake the outside references behind the flows-spec harness design
- Build the flows genre; collapse capture docs into meta/flows/session-capture
...
- Intake first concept; add claim/link types, provenance/verified fields
- Drop the link type; links enter only once processed into a reference
- Make CLAUDE.md a compiled artifact; add meta/ governance + Elixir compiler
- Add SessionStart hook to install Elixir in web sessions
- Add CI workflow and optional pre-commit hook
- Add stable-id registry and grounded verification; verify the git claim
- Archive bootstrap thread in meta/threads/
...
- 2026-03-03 — Add publish pipeline: blog posts from assertions, research from sources
- 2026-03-03 — Add persistent conversations: context-load, thread, export
- 2026-03-03 — Add --describe JSON schema to all scripts, test runner
- 2026-03-03 — Add LLM call abstraction, JSONL logging, and test harness
- 2026-03-03 — semantic query layer plan
- 2026-03-03 — Ingest CLI-MCP conversation and Holmes article, add memory policy plan
- 2026-03-03 — Document inline evidence convention in schema, add bash 3 note to memory
- 2026-03-03 — cli-mcp-spce and intake
- 2026-03-03 — First /conclude: extract assertions from persistence architecture research
- 2026-03-03 — Add persistence research to plan 002, source fidelity rule to /conclude
- 2026-03-03 — Move plan 001 to history, update plan 002 for DAG architecture
- 2026-03-03 — Add plan 003 document (assertion DAG knowledge architecture)
- 2026-03-03 — intake added
- 2026-03-03 — Implement assertion DAG knowledge architecture (plan 003)
- 2026-03-02 — Merge worktree: Ingest macOS disk cleanup tips from HN thread
- 2026-02-25 — Update CLAUDE.md with ingestion notes for Reddit, X, YouTube
- 2026-02-25 — Ingest longform Claude Code guide, cross-link with repo article
- 2026-02-25 — Add narrative summary to agentic coding ep3 note
- 2026-02-25 — Ingest first video: State of Agentic Coding #3 (Armin & Ben)
- 2026-02-25 — Fix article to use OpenClaw, add Reddit support to ingest.sh
- 2026-02-25 — Ingest first article: Mem0 persistent memory plugin for Cline
- 2026-02-25 — Add conversation content type and first archived investigation
- 2026-02-25 — Add initial scaffold: templates, scripts, and directory structure
- 2026-02-25 — Add CLAUDE.md with project overview and next steps
- 2026-02-25 — Add initial second brain architecture plan
