...
provenance: "Claude Code session (Claude Opus 4.8), 2026-07-12 — seeded from the 14 probes of the vector-DB recall analysis, re-adjudicated to em: ids per the dedup-recall-probe plan"
...
re-adjudicated to stable `em:` ids. That analysis ran the probe **once, by hand**,
...
| **acceptable ids** | one or more `em:` ids; the row **hits** if the lexical backend surfaces *any* of them (query-relativity: several concepts can legitimately answer one phrasing) |
...
|-------|----------------|------|----------|------|
| context pollution | em:784985 | target | context poisoning; logical context poisoning; poisoned context | The hypothesized miss: the concept titles the failure "poisoning", never "pollution", so plain search returns unrelated files. Recovered by the "poisoning" variant. |
| lost in the middle | em:77d68a | target | context rot; long-context degradation; positional degradation | Canonical synonym gap: "lost in the middle" and "context rot" share zero tokens. The variant "context rot" (the concept's own term) recovers it. |
| long context degradation | em:77d68a | target | context rot; degrades over long context | Same target, a second natural phrasing; the "context rot" variant recovers it. |
| stale branch | em:4c9e1f | target | auto-advance; local branch behind; branch didn't move on fetch | Concept is titled "…don't auto-advance on fetch"; the word "stale" never appears. The "auto-advance" variant recovers it. |
| branch not updating | em:4c9e1f | target | auto-advance; fetch doesn't move the branch | Same target, a second phrasing; recovered by "auto-advance". |
| codebase graph | em:b89ea1 | target | code knowledge graph; knowledge graph of the code | GitNexus says "code knowledge graph"; the transposed "codebase graph" misses. The exact-order variant recovers it. |
| inference margin | em:07610c | target | inference gross margin; gross margin | The concept uses "inference gross margins"; the bare "inference margin" phrase lands only in the (excluded) index. The full-phrase variant recovers it. |
...
1. **Harvests a gold row** — if the intake carried a natural phrasing (the operator's
   ...
   filed/merged concept's `em:` id, using that **actual** phrasing (never a synthetic
...
- **2026-07-12** — seeded from the vector-DB recall analysis's 14 probes,
  re-adjudicated to `em:` ids. The analysis's combined "found" row
