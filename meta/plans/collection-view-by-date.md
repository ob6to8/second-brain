...
status: done
provenance: "Claude Code session, 2026-07-13 — operator question ('how might we view a collection according to date modified/created? Say for instance, /plans'), agent survey of the brain's date signals and rendering surfaces, operator ratified `timestamp` as the signal and the two least-work items"
tags: [meta, plan, indexes, collections, recency, timestamp]
timestamp: 2026-07-13
attribution:
  when: 2026-07-13T16:27:22Z
...
Executed alongside: one drift fix surfaced by the reorder — the dedup-recall-probe
plan was listed under "Proposed" while its `status` was `done`; its entry moved to
the Done section.
...
- **A. `mix brain.recent <dir> [--by created|modified] [--limit N]`** — a
  read-only task reusing `SecondBrain.Frontmatter.parse/1`: sort by `timestamp`
...
- **C. Site-side sort toggle** — the generated Pages site already renders
  `timestamp` as "Updated" on each concept page; generated directory pages could
  grow a client-side sort-by-date control. Browsing nicety, no in-session value.
