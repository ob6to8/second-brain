...
- [Rename the brain: second-brain → elixir-brain (including sb: → eb: id migration)](/meta/plans/rename-second-brain-to-elixir-brain.md) — spec for renaming the repo and all associated naming, rescoped by the operator to also migrate the stable-id prefix `sb:` → `eb:`. Two atomic PRs: workstream A (Elixir app/module namespace, hook, maintained docs, regenerated artifacts, frozen-thread `<routes>` path refs) and workstream B (a tail-preserving, bundle-wide `sb:X → eb:X` prefix swap across 200 minted ids and ~940 tokens, code/verifier flip, stable-identity policy amendment), plus an operator repo-rename on GitHub. Ratification covers the policy amendment and two scoped frozen-body exceptions. `status: proposed`.
