...
- [Glossary: single canonical overview per term, dedup'd bodies, and a mechanical check](/meta/plans/glossary-single-overview-and-dedup-check.md) — each term's `description` becomes the one canonical overview (entry lede + index gloss verbatim); all 148 bodies rewritten to expansion-only; `mix brain.glossary` enforces it (index `## Terms` materialized like the registry, near-restatement sentences fail via content-word containment, moderate overlap warns), gating CI and the Pages deploy. `status: done`.
