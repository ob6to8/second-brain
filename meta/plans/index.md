...
- [Attribution: a frontmatter record of the ingestion event](/meta/plans/resource-attribution-property.md) — add a structured `attribution` property to bundle concepts **and governance docs** recording when an entry was ingested, through which channel and by whom, and why it was filed — plus, on governance docs, an explicit `from` back-link to the thread or concept it was extracted from (subsuming the elaboration `thread:` field): an immutable event record orthogonal to `provenance` (content origin) and `timestamp` (last change), with verifier shape rules, skill writers, a git-derived backfill, and an optional `mix brain.attribution` query surface. `status: proposed`.
