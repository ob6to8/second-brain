...
- [Concept terminology: the document/concept split (done) and the concept-type redefinition (open)](/meta/plans/concept-terminology-and-type-redefinition.md) — record of the ratified "document"-for-the-unit rename (commit `fe5aa52`), plus the finding that `type: concept` is two speech acts sharing a name — term definition (149 glossary entries, unverified) and accepted proposition (one graduated claim) — under an "(established/accepted)" label no tooling checks; ends in the open questions (own type for glossary terms? drop claim→concept graduation? drop or enforce the parenthetical?) awaiting operator ratification. `status: proposed`.
