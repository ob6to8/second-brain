...
- [Attribution: a frontmatter record of the ingestion event](/meta/plans/resource-attribution-property.md) — add a structured `attribution` property to bundle concepts **and governance docs** recording when an entry was ingested, through which channel and by whom, and why it was filed — plus, on governance docs, an append-only `from` back-link to the thread or concept it was extracted from (subsuming the elaboration `thread:` field and, post-backfill, the hand-kept flow `lineage:` blocks, which `mix brain.lineage` will derive from `from` + `pr:`): an immutable event record orthogonal to `provenance` (content origin) and `timestamp` (last change), with verifier-mandatory presence after a git-derived backfill, skill writers, immediate `auto-intake`-tag retirement, and a `mix brain.attribution` query surface. All five open questions ratified 2026-07-13. `status: accepted`.
